...
static bool init_units_characteristics(struct jni *jni,
                                       jobjectArray characteristics_array);

static bool load_combatants(struct jni *jni, struct combatant **combatants,
                            size_t *num_combatants,
                            jobjectArray combatants_array);

static jobject create_battle_outcome(struct jni *jni,
                                     const struct combatant *restrict attackers,
                                     size_t num_attackers,
                                     const struct combatant *restrict defenders,
                                     size_t num_defenders, size_t num_rounds);

static jobjectArray
create_combatants_outcomes(struct jni *jni, const struct combatant *combatants,
                           size_t num_combatants, size_t num_rounds);

static jobject create_combatant_outcome(struct jni *jni,
                                        const struct combatant *combatant,
                                        size_t num_rounds);

static size_t fight(struct combatant *restrict attackers, size_t num_attackers,
                    struct combatant *restrict defenders, size_t num_defenders,
                    uint32_t *random);

static struct party *create_party(struct combatant *combatants,
                                  size_t num_combatants);

static void restore_shields(struct party *party);

static void fire(struct party *restrict attackers_party,
                 struct party *restrict defenders_party, size_t round,
                 uint32_t *random);

static void update_units(struct combatant *combatants, struct party *party,
                         size_t round);

static void update_combatants(struct combatant *combatants,
                              size_t num_combatants, struct party *party);
...
Java_com_github_retro_1game_retro_1game_battleengine_BattleEngine_init(
  ...
  (void)battle_engine;
  struct jni jni;
  return init_jni(&jni, env) &&
         init_units_characteristics(&jni, characteristics_array);
}
...
Java_com_github_retro_1game_retro_1game_battleengine_BattleEngine_fight(
  ...
  size_t num_rounds =
      fight(attackers, num_attackers, defenders, num_defenders, &random);

  jobject battle_outcome = create_battle_outcome(
      &jni, attackers, num_attackers, defenders, num_defenders, num_rounds);
...
bool init_jni(struct jni *jni, JNIEnv *env) {
  jni->env = env;
  return init_jni_battle_outcome(jni) && init_jni_combatant(jni) &&
         init_jni_combatant_outcome(jni) && init_jni_unit_characteristics(jni);
}

bool init_jni_battle_outcome(struct jni *jni) {
  jni->battle_outcome.class =
      (*jni->env)->FindClass(jni->env, BATTLE_OUTCOME_CLASS);
  if (jni->battle_outcome.class == NULL) {
    return false;
  }
  jni->battle_outcome.constructor = (*jni->env)->GetMethodID(
      jni->env, jni->battle_outcome.class, "<init>",
...
bool init_jni_combatant(struct jni *jni) {
  ...
  jni->combatant.get_weapons_technology = (*jni->env)->GetMethodID(
      jni->env, jni->combatant.class, "getWeaponsTechnology", "()I");
  jni->combatant.get_shielding_technology = (*jni->env)->GetMethodID(
      jni->env, jni->combatant.class, "getShieldingTechnology", "()I");
  jni->combatant.get_armor_technology = (*jni->env)->GetMethodID(
      jni->env, jni->combatant.class, "getArmorTechnology", "()I");
  jni->combatant.get_unit_groups = (*jni->env)->GetMethodID(
      jni->env, jni->combatant.class, "getUnitGroups", "()[I");
  return jni->combatant.get_weapons_technology != NULL &&
         jni->combatant.get_shielding_technology != NULL &&
         jni->combatant.get_armor_technology != NULL &&
         jni->combatant.get_unit_groups != NULL;
}

bool init_jni_combatant_outcome(struct jni *jni) {
  jni->combatant_outcome.class =
      (*jni->env)->FindClass(jni->env, COMBATANT_OUTCOME_CLASS);
  if (jni->combatant_outcome.class == NULL) {
    return false;
  }
  jni->combatant_outcome.constructor = (*jni->env)->GetMethodID(
      jni->env, jni->combatant_outcome.class, "<init>", "(I[J)V");
  return jni->combatant_outcome.constructor != NULL;
}

bool init_jni_unit_characteristics(struct jni *jni) {
  jni->unit_characteristics.class =
      (*jni->env)->FindClass(jni->env, UNIT_CHARACTERISTICS_CLASS);
  if (jni->unit_characteristics.class == NULL) {
    return false;
  }
  jni->unit_characteristics.get_weapons = (*jni->env)->GetMethodID(
      jni->env, jni->unit_characteristics.class, "getWeapons", "()F");
  jni->unit_characteristics.get_shield = (*jni->env)->GetMethodID(
      jni->env, jni->unit_characteristics.class, "getShield", "()F");
  jni->unit_characteristics.get_armor = (*jni->env)->GetMethodID(
      jni->env, jni->unit_characteristics.class, "getArmor", "()F");
  jni->unit_characteristics.get_rapid_fire = (*jni->env)->GetMethodID(
      jni->env, jni->unit_characteristics.class, "getRapidFire", "()[I");
  return jni->unit_characteristics.get_weapons != NULL &&
         jni->unit_characteristics.get_shield != NULL &&
         jni->unit_characteristics.get_armor != NULL &&
         jni->unit_characteristics.get_rapid_fire != NULL;
}

bool init_units_characteristics(struct jni *jni,
                                jobjectArray characteristics_array) {
  size_t num_elems =
      (size_t)(*jni->env)->GetArrayLength(jni->env, characteristics_array);

  size_t total_size = num_elems * sizeof(struct unit_characteristics) +
                      num_elems * num_elems * sizeof(int);
  ...
  for (size_t i = 0; i < num_elems; i++, rf += num_elems) {
    jobject object = (*jni->env)->GetObjectArrayElement(
        jni->env, characteristics_array, (jsize)i);
    if (object == NULL) {
      free(characteristics);
      return false;
    }

    jobject rf_array_object = (*jni->env)->CallObjectMethod(
        jni->env, object, jni->unit_characteristics.get_rapid_fire);
    ...
    int *array =
        (*jni->env)->GetIntArrayElements(jni->env, rf_array_object, NULL);
    ...
    c->weapons = (*jni->env)->CallFloatMethod(
        jni->env, object, jni->unit_characteristics.get_weapons);
    c->shield = (*jni->env)->CallFloatMethod(
        jni->env, object, jni->unit_characteristics.get_shield);
    c->armor = (*jni->env)->CallFloatMethod(
        jni->env, object, jni->unit_characteristics.get_armor);
    c->rapid_fire = rf;
  }
...
bool load_combatants(struct jni *jni, struct combatant **combatants,
                     size_t *num_combatants, jobjectArray combatants_array) {
  size_t num_elems =
      (size_t)(*jni->env)->GetArrayLength(jni->env, combatants_array);
  size_t total_size =
      num_elems * sizeof(struct combatant) +
      num_elems * num_kinds * sizeof(int) +
  ...
  struct unit_group_stats *stats =
      (struct unit_group_stats *)&unit_groups[num_elems * num_kinds];
  ...
  for (size_t i = 0; i < num_elems;
    ...
    jobject object = (*jni->env)->GetObjectArrayElement(
        jni->env, combatants_array, (jsize)i);
    if (object == NULL) {
      free(cs);
      return false;
    }

    jobject array_object = (*jni->env)->CallObjectMethod(
        jni->env, object, jni->combatant.get_unit_groups);
    ...
    c->weapons_technology = (uint8_t)(*jni->env)->CallIntMethod(
        jni->env, object, jni->combatant.get_weapons_technology);
    c->shielding_technology = (uint8_t)(*jni->env)->CallIntMethod(
        jni->env, object, jni->combatant.get_shielding_technology);
    c->armor_technology = (uint8_t)(*jni->env)->CallIntMethod(
        jni->env, object, jni->combatant.get_armor_technology);
...
jobject create_battle_outcome(struct jni *jni,
                              const struct combatant *restrict attackers,
                              size_t num_attackers,
                              const struct combatant *restrict defenders,
                              size_t num_defenders, size_t num_rounds) {
  jobjectArray attackers_array =
      create_combatants_outcomes(jni, attackers, num_attackers, num_rounds);
  jobjectArray defenders_array =
      create_combatants_outcomes(jni, defenders, num_defenders, num_rounds);
  return (*jni->env)->NewObject(jni->env, jni->battle_outcome.class,
                                jni->battle_outcome.constructor, num_rounds,
                                attackers_array, defenders_array);
}

jobjectArray create_combatants_outcomes(struct jni *jni,
                                        const struct combatant *combatants,
                                        size_t num_combatants,
                                        size_t num_rounds) {
  jobjectArray array = (*jni->env)->NewObjectArray(
      jni->env, (jsize)num_combatants, jni->combatant_outcome.class, NULL);
...
jobject create_combatant_outcome(struct jni *jni,
                                 const struct combatant *combatant,
                                 size_t num_rounds) {
  size_t length =
      num_rounds * num_kinds * (sizeof(*combatant->stats) / sizeof(long));
  jlongArray stats = (*jni->env)->NewLongArray(jni->env, (jsize)length);
  (*jni->env)->SetLongArrayRegion(jni->env, stats, 0, (jsize)length,
                                  (long *)combatant->stats);
  return (*jni->env)->NewObject(jni->env, jni->combatant_outcome.class,
                                jni->combatant_outcome.constructor, num_kinds,
                                stats);
}

size_t fight(struct combatant *restrict attackers, size_t num_attackers,
             struct combatant *restrict defenders, size_t num_defenders,
             uint32_t *random) {
  size_t round = 0;
...
struct party *create_party(struct combatant *combatants,
                           size_t num_combatants) {
  ...
  for (size_t i = 0; i < num_combatants; i++) {
    ...
    for (uint8_t kind = 0; kind < num_kinds; kind++) {
      float max_hull = 0.1f * units_characteristics[kind].armor *
...
void restore_shields(struct party *party) {
  ...
  for (size_t i = 0; i < party->num_alive; i++) {
    ...
    unit->shield =
        units_characteristics[unit->kind].shield *
        (1.0f + 0.1f * combatants[unit->combatant_id].shielding_technology);
  }
}

void fire(struct party *restrict attackers_party,
          struct party *restrict defenders_party, size_t round,
          uint32_t *random) {
  uint32_t r = *random;
  ...
  for (size_t i = 0; i < num_shooters; i++) {
    ...
    float damage =
        shooter_characteristics->weapons *
    ...
    do {
      ...
      const struct unit_characteristics *target_characteristics =
          &units_characteristics[target_kind];
      ...
      if (target->hull != 0.0f) {
        ...
        if (hull_damage < 0.0f) {
          float max_shield = target_characteristics->shield *
        ...
        if (hull != 0.0f) {
          float max_hull = 0.1f * target_characteristics->armor *
...
void update_combatants(struct combatant *combatants, size_t num_combatants,
                       struct party *party) {
  for (size_t i = 0; i < num_combatants; i++) {
